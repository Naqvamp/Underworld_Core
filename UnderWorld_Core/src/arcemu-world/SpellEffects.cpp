	...
	else
	{
		...
		else
		{
			if( u_caster != NULL )
			{
				...
				switch (GetProto()->NameHash)
				{
				case 0xc5811dec: //Swipe(bear)
					u_caster->Strike( unitTarget, _type, GetProto(), 0, 0, dmg, pSpellId == 0, true, force_crit, true );
					return;
				}
...
void Spell::SpellEffectTeleportUnits( uint32 i )  // Teleport Units
{
	...
	uint32 spellId = GetProto()->Id;
	Log.Error("DEBUG/SPELL", "tele units spell id: %u", spellId);
...
void Spell::SpellEffectWeaponDmgPerc(uint32 i) // Weapon Percent damage
{
	...
	else
	{
		...
		switch (GetProto()->NameHash)
		{
		case SPELL_HASH_DIVINE_STORM:
		case SPELL_HASH_DIVINE_STORM_DAMAGE:
		case SPELL_HASH_FAN_OF_KNIVES:
			u_caster->Strike( unitTarget, _type, GetProto(), add_damage, damage, 0, false, true, true );
			return;
		}
	...
	case LOCKTYPE_SLOW_OPEN: // used for BG go's
		{
			...
			if(p_caster)
			{
				...
				{
					if(sGWarMgr.AssaultNode(gameObjTarget->bannerslot, p_caster))
						return;
				}
			}
			
			uint32 spellid = gameObjTarget->GetInfo()->Unknown1;
			if (!spellid)
				return;
...
void Spell::SpellEffectDispel(uint32 i) // Dispel
{
	...
	for(uint32 x = start; x<end; x++)
		if(unitTarget->m_auras[x])
		{
			...
			//no dispelling the gm freeze auras >:D
...
void Spell::SpellEffectStuck(uint32 i)
{
	if(!playerTarget || playerTarget != p_caster)
		return;
	if (p_caster->GetMapId() == 44)
	{
		sChatHandler.RedSystemMessageToPlr(p_caster, "ERROR: Please direct your attention to the staff member who brought you here.");
		return;
	}
	...
	sEventMgr.AddEvent(playerTarget,&Player::EventTeleport, Map, X, Y, Z,
...
void Spell::SpellEffectDummyMelee( uint32 i ) // Normalized Weapon damage +
{
	...
	for (uint32 x = 0;x<3;x++)
		...
		switch (GetProto()->NameHash)
		{
		case SPELL_HASH_WHIRLWIND:
		case SPELL_HASH_BLADESTORM:
			u_caster->Strike( unitTarget, _type, GetProto(), damage, pct_dmg_mod, 0, false, true, true );
			return;
		}
