...
// Last edited by:	$Author: Hypersniper $
// revision:		$Rev: 3131 $
// date:			$Date: 2010-01-25 23:22:31 -0500 (Mon, 25 Jan 2010) $
...
AddItemResult ItemInterface::m_AddItem(Item *item, int8 ContainerSlot, int16 slot)
{
	...
	if (m_pOwner->bGMTagOn)
	{
		sChatHandler.RedSystemMessageToPlr(m_pOwner, "ERROR: You cannot receive items while your tag is on. Remove it to be able to do so.");
		return ADD_ITEM_RESULT_ERROR;
	}
...
{
	if(pVendor->GetEntry() >= 59000 && pVendor->GetEntry() <= 59999)
	{
		uint32 pCost = 0;
		uint32 entry = pVendor->GetEntry();
		...
		{
			char purchase[1024];
			snprintf(purchase,1024,"ITEM: %s(%u)", item->Name1, item->ItemId);
			m_pOwner->GetSession()->LogPurchase(purchase,(pCost*total_amount));
			return;
		}
	}
	/*else if( item->ItemId > 44999 && item->ItemId < 46000 )
	{
		uint32 uwprice=item->ItemLevel;
		if (m_pOwner->GetSession()->m_points >= (total_amount * uwprice))
		{
			char purchase[1024];
			snprintf(purchase,1024,"ITEM: %s(%u)", item->Name1, item->ItemId);
			m_pOwner->GetSession()->LogPurchase(purchase,(uwprice*total_amount));
		}
		else
		{
			sChatHandler.RedSystemMessageToPlr(m_pOwner, "ERROR: You need %u more points for this purchase.", (total_amount * uwprice) - m_pOwner->GetSession()->m_points);
			return;
		}
	}*/
	if(item->BuyPrice)
...
int8 ItemInterface::CanAffordItem(ItemPrototype * item,uint32 amount, Creature * pVendor)
{
	if(pVendor->GetEntry() >= 59000 && pVendor->GetEntry() <= 59999)
	{
		...
		if(pCost > 0)
		{
			if( m_pOwner->GetSession()->m_points < ( pCost * amount) )
			{
				uint32 pointsneeded = (( pCost * amount) - m_pOwner->GetSession()->m_points);
				sChatHandler.RedSystemMessageToPlr(m_pOwner, "ERROR: You need %u more Points for this purchase.", pointsneeded);
	...
	if( item->ItemId > 44999 && item->ItemId < 46000 )
	{
		uint32 uwprice=item->ItemLevel;
		if (m_pOwner->GetSession()->m_points < (amount * uwprice))
		{
			sChatHandler.RedSystemMessageToPlr(m_pOwner, "ERROR: You need %u more points for this purchase.", (amount * uwprice) - m_pOwner->GetSession()->m_points);
			return CAN_AFFORD_ITEM_ERROR_NOT_ENOUGH_MONEY;
		}
	}
