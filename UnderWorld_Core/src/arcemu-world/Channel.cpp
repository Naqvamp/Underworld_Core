...
void Channel::AttemptJoin(Player * plr, const char * password)
{
	...
	if(plr->GetSession()->m_gmData->rank >= RANK_ADMIN ) //admins mod every channel
...
void Channel::List(Player * plr)
{
	...
	for(MemberMap::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
	{
		...
		if(itr->first)
		{
			if(itr->first->m_isGmInvisible && plr->GetSession()->m_gmData->rank < RANK_ADMIN)
				continue;
			
			if(itr->first->GetSession()->m_gmData->rank == RANK_SYRA && plr->GetSession()->m_gmData->rank < RANK_ADMIN)
				continue;
		}
