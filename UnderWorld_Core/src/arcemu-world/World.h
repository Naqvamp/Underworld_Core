...
public:
	...
	//UNDERWORLD
	void SendWorldText_UW(const char *text, ...);
	uint32 ann_cd;
	uint32 wann_cd;
	void ShowUsersWithAccount(const char * account, WorldSession * m_session);
	void ShowUsersWithIP(const char * ip, WorldSession * m_session);
	void ToggleArenaProgress(WorldSession * m_session, bool state);
	uint32 arena1MinKills;
	uint32 arena2MinKills;
	uint32 arena3MinKills;
	uint32 addHonorHardCap;
	uint32 addKillsHardCap;
	bool arenaEventInProgress;
	uint32 arenaEventTimeout;
	bool blockOutlands;
	uint32 customItemCutoff;
	bool enableIntroVid;
	string logonDB;
	uint32 resilCap;
	uint32 realmID;
	...
	float ap_modifier;
	float sp_modifier;
	uint32 dark_light_enabled;
	uint32 dark_lang;
	uint32 light_lang;
	uint32 m_usePvPTokens;
	float dodge_ratio_level;
	float dodge_ratio_agi;
	uint32 spell_haste_cap;
	uint32 dmg_cap;
};
