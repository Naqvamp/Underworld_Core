...
//		sp->Effect[0] = SPELL_EFFECT_POWER_BURN; // should be Power Burn, not Power Drain. Power Drain leeches mana which is incorrect.
	...
	**********************************************************/
	...
	// Insert warrior spell fixes here
		...
		sp = dbcSpell.LookupEntryForced( 5599 );
		if (sp != NULL)
			sp->targetAuraSpellNot = 25771;

		//Hand of Protection Rank 3
		sp = dbcSpell.LookupEntryForced( 10278 );
		if (sp != NULL)
			sp->targetAuraSpellNot = 25771;
	...
	/*	// !!! not sure this is good !!! have to test
		...
		sp = dbcSpell.LookupEntryForced( 63737 ); // Rank 3
		if( sp != NULL )
		{
			   sp->procFlags = PROC_ON_CAST_SPELL;
			   sp->ProcOnNameHash[0] = SPELL_HASH_BINDING_HEAL;
...
// #endif
	...
	//UNDERWORLD Fixes
	//30 second invis
	sp = dbcSpell.LookupEntryForced( 12845 );
	if (sp != NULL)
		sp->c_is_flags |= SPELL_FLAG_IS_FORCEDDEBUFF;

	//30 second banish
	sp = dbcSpell.LookupEntryForced( 42354 );
	if (sp != NULL)
		sp->c_is_flags |= SPELL_FLAG_IS_FORCEDDEBUFF;

	//Frezeze aura
	sp = dbcSpell.LookupEntryForced( 9454 );
	if (sp != NULL)
		sp->c_is_flags |= SPELL_FLAG_IS_FORCEDDEBUFF;

	//Timeoutaura
	sp = dbcSpell.LookupEntryForced( 17691 );
	if (sp != NULL)
		sp->c_is_flags |= SPELL_FLAG_IS_FORCEDDEBUFF;
	...
	if (sp != NULL)
		sp->RecoveryTime = 10000;
	
	//Chaos flames
	sp = dbcSpell.LookupEntryForced( 39055 );
	if (sp != NULL)
		sp->RecoveryTime = 30000;
	...
	//10 sec banish (for pvp deflagging)
	...
	// chain Fire Ball
	sp = dbcSpell.LookupEntryForced( 35853 );
	if( sp != NULL )
	{
		sp->RecoveryTime = 120000;
	}
