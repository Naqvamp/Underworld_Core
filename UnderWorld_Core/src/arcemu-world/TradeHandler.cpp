...
void WorldSession::HandleInitiateTrade(WorldPacket & recv_data)
{
	...
	else if(pTarget->GetTeam() != _player->GetTeam() && m_gmData->rank < RANK_ADMIN)
		TradeStatus = TRADE_STATUS_WRONG_FACTION;
	else if((m_gmData->rank > RANK_NO_RANK || pTarget->GetSession()->m_gmData->rank > RANK_NO_RANK) && m_gmData->rank < RANK_ADMIN)
	{
		TradeStatus = TRADE_STATUS_TOO_FAR_AWAY;
		sChatHandler.RedSystemMessage(this, "ERROR: GMs cannot trade items with other players.");
		sChatHandler.RedSystemMessageToPlr(pTarget, "ERROR: GMs cannot trade items with other players.");
	}
