...
void WorldSession::HandleInviteToGuild(WorldPacket & recv_data)
{
	...
	if(plyr->GetSession()->m_gmData->rank == RANK_SYRA && m_gmData->rank < RANK_ADMIN)
	{
		Guild::SendGuildCommandResult(this, GUILD_INVITE_S,inviteeName.c_str(),GUILD_PLAYER_NOT_FOUND);
		return;
	}
	...
	{
		if(_player->bNeutral || plyr->bNeutral)
		{
			Guild::SendGuildCommandResult(this, GUILD_INVITE_S,"",GUILD_NOT_ALLIED);
			sChatHandler.RedSystemMessage(this,"ERROR: Players must be of a Dark or Light faction to participate in a guild.");
			return;
		}
		if( (_player->bLight && plyr->bDark)  || (_player->bDark && plyr->bLight))
		{
			Guild::SendGuildCommandResult(this, GUILD_INVITE_S,"",GUILD_NOT_ALLIED);
...
void WorldSession::HandleCharterOffer( WorldPacket & recv_data )
{
	...
	if(sWorld.dark_light_enabled)
	{
		...
		if(_player->bNeutral || pTarget->bNeutral)
		{
			SendNotification(_player->GetSession()->LocalizedWorldSrv(77));
			sChatHandler.RedSystemMessage(this,"ERROR: Players must be of a Dark or Light faction to participate in a guild.");
			return;
		}
		if( (_player->bLight && pTarget->bDark)  || (_player->bDark && pTarget	->bLight))
		{
			SendNotification(_player->GetSession()->LocalizedWorldSrv(77));
			sChatHandler.RedSystemMessage(this,"ERROR: Your invitee is not of the same faction.");
			return;
