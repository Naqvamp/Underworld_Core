...
bool ChatHandler::HandleGMOnCommand(const char* args, WorldSession *m_session)
{
	...
	else
	{
		if(!RANK_CHECK(RANK_COADMIN))
		{
			...
			{
				RedSystemMessage(m_session, "You are flagged for player combat and cannot turn your GM Tag on.  You must activate your GM tag prior to your PvP flag if you wish to .gmon.");
				return true;
			}
		}
		
		_player->m_GMTagOff = ((uint32)UNIXTIME + 120);
...
bool ChatHandler::HandleGMOffCommand(const char* args, WorldSession *m_session)
{
	...
	{
		if(!RANK_CHECK(RANK_COADMIN) && (m_session->GetPlayer()->m_GMTagOff > (uint32)UNIXTIME))
		{
			uint32 timeleft = (m_session->GetPlayer()->m_GMTagOff - (uint32)UNIXTIME);
			RedSystemMessage(m_session,"ERROR: You must wait at least 2 minutes after using .gm on to turn it off. %u seconds left.", timeleft);
			return true;
		}
		...
		{
			BlueSystemMessage(m_session, "You are in a restricted area and do not meet the requirements to be in here.");
			_player->SafeTeleport(_player->GetBindMapId(), 0, _player->GetBindPositionX(), _player->GetBindPositionY(), _player->GetBindPositionZ(), 0);
			return true;
		}
		...
		if (_player->IsPvPFlagged())
		{
			RedSystemMessage(m_session, "ERROR: PvP Flag detected.");
			_player->Neutralize(false);
		}
	}
...
bool ChatHandler::HandleAddInvItemCommand(const char *args, WorldSession *m_session)
{
	...
	{
		if((sWorld.customItemCutoff > 0 && itemid > sWorld.customItemCutoff) || itemid == 29434 || itemid == 11078)//Make sure they dont try to add custom items like admin gear
		{
			RedSystemMessage(m_session, "You may not use this command to obtain custom items.");
			return true;
		}

		//Place any specific items here that we dont want GMs to have i.e. "Martin Thunder"
		// 192		= Martin Thuner
		// 5417		= WMD
		// 5418		= WMD
		// 28132	= Area 52 Special
		if( itemid == 192 || itemid == 5417 || itemid == 5418 || itemid == 28132)
		{
			RedSystemMessage(m_session, "ERROR: Did you really think we'd let you add insta-kill items to your inventory?");
			return true;
		}
	}
	...
	if(it)
	{
		numadded -= plr->GetItemInterface()->GetItemCount( itemid );
        bool result = false;
        result = plr->GetItemInterface()->AddItemById( itemid, count, randomprop );
		numadded += plr->GetItemInterface()->GetItemCount( itemid );
        if( result == true ){
            
            char messagetext[512];
            
            snprintf(messagetext, 512, "Added item %s (id: %d), quantity %u, to %s's inventory.", GetItemLinkByProto(it, m_session->language).c_str(), (unsigned int)it->ItemId, numadded, plr->GetName());
            SystemMessage(m_session, messagetext);
        }
		if (plr != m_session->GetPlayer())
		{
			char notes[1024];
			snprintf(notes, 1024, "ID: %u [%s] - COUNT: %u", it->ItemId, it->Name1, count);
			GMLog(m_session, plr->GetSession(), "additem", notes);
		}
        return true;
...
bool ChatHandler::HandleSummonCommand(const char* args, WorldSession *m_session)
{
	...
	*/
	...
	if (chr)
	{
		...
		char buf[256];
		char buf0[256];
		if (!EnoughRankCheck(m_session->GetPlayer(), chr))
			return true;
		...
		if (chr->DuelingWith)
		{
			RedSystemMessage(m_session, "ERROR: That player is dueling.");
			return true;
		}

		if (chr->GetMapId() == 44 && !RANK_CHECK(RANK_COADMIN))
		{
			snprintf(buf, 256, "%s is trying to summon me!.", m_session->GetPlayer()->GetName());
			WorldPacket * data = FillMessageData(CHAT_MSG_SAY, LANG_UNIVERSAL, args, chr->GetGUID(), 0);
	...
	else
	{
		...
		else
		{
			if (!RANK_CHECK(RANK_COADMIN))
			{
				QueryResult * res = CharacterDatabase.Query("SELECT mapId FROM characters WHERE guid = %u", pinfo->guid);
				if (res)
				{
					if (res->Fetch()[0].GetUInt32() == 44) //Jail!
					{
						RedSystemMessage(m_session, "ERROR: You cannot remove a player from the jail.");
						return true;
					}
...
*/
	...
	if (chr)
	{
		...
		if (!m_session->GetPlayer()->AllowedPortTo(chr->GetMapId(), true))
			return true;

		char buf[256];

		if (chr->bADMINTagOn && chr->GetSession()->m_gmData->rank == RANK_SYRA)
		{
			SystemMessage(m_session, "Player (%s) does not exist or is not logged in.", chr->GetName());
			return true;
		}

		if (chr->bADMINTagOn && !RANK_CHECK(RANK_ADMIN))
		{
			SystemMessage(m_session, "Player (%s) does not exist or is not logged in.", chr->GetName());
			return true;
		}
...
bool ChatHandler::HandleTaxiCheatCommand(const char* args, WorldSession *m_session)
{
	...
	{
		GreenSystemMessage(m_session, "%s has all taxi nodes now.", plr->GetName());
	}
	else if(stricmp(args, "off") == 0)
	{
		GreenSystemMessage(m_session, "%s has no more taxi nodes now.", plr->GetName());
	}
	else
		return false;
...
bool ChatHandler::HandleModifySpeedCommand(const char* args, WorldSession *m_session)
{
	...
	float cap = (float)(20 + (10 * m_session->m_gmData->rank) + (RANK_CHECK(RANK_ADMIN) ? 60 : 0));
	...
	if (Speed > cap)
	{
		RedSystemMessage(m_session, "ERROR: Incorrect value. Set to rank cap.");
		Speed = cap;
	}
	...
	{
		char notes[50];
		snprintf(notes, 50, "speed %u", Speed);
		GMLog(m_session, plr->GetSession(), "mofify speed", notes);

		BlueSystemMessage(m_session, "You set the speed of %s to %2.2f.", plr->GetName(), Speed);
		GreenSystemMessageToPlr(plr, "%s set your speed to %2.2f.", m_session->GetPlayer()->GetName(), Speed);
	}
	else
		BlueSystemMessage(m_session, "You set your speed to %2.2f.", Speed);

	plr->SetPlayerSpeed(RUN, Speed);
	plr->SetPlayerSpeed(SWIM, Speed);
	plr->SetPlayerSpeed(RUNBACK, Speed / 2); // Backwards slower, it's more natural :P
	plr->SetPlayerSpeed(FLY, Speed * (RANK_CHECK(RANK_ADMIN) ? 2 : 1.25F)); // Flying is faster :P
...
//	WorldPacket data;
	...
	{
		char notes[50];
		snprintf(notes, 50, "%d gold", total);
		GMLog(m_session, plr->GetSession(), "modify gold", notes);
	}

	int32 newgold = plr->GetGold() + total;

	// Check they don't have more than the max gold
	if(sWorld.GoldCapEnabled)
    {
        if(newgold > (int32)sWorld.GoldLimit)
        {
				RedSystemMessage(m_session, "Maximum amount of gold is %u and %s already has %u", (sWorld.GoldLimit/10000), plr->GetName(), (plr->GetGold()/10000));
            return true;
        }
    }
	...
	if(newgold < 0)
	{
		BlueSystemMessage( m_session, "Taking all gold from %s's backpack...", plr->GetName() );
		GreenSystemMessageToPlr(plr, "%s took the all gold from your backpack.", m_session->GetPlayer()->GetName());
		newgold = 0;
	}
	else
	{
		if(total >= 0) {
			...
			GreenSystemMessageToPlr( plr, "%s added %u gold, %u silver, %u copper to your backpack.",
		...
		else
		{
			...
			GreenSystemMessageToPlr( plr, "%s took %u gold, %u silver, %u copper from your backpack.",
				m_session->GetPlayer()->GetName(),
				gold, silver, copper );
		}
	}
...
bool ChatHandler::HandleModifyTPsCommand(const char* args, WorldSession *m_session)
{
	...
	if(TP1)
		plr->m_specs[SPEC_PRIMARY].m_customTalentPointOverride = TP1;
	if(TP2)
		plr->m_specs[SPEC_SECONDARY].m_customTalentPointOverride = TP2;
