...
public:
	...
	bool hiding;
	uint32 m_nextKillC;
	bool MassSumEligible;
	ARCEMU_INLINE uint32 pAcctId() { return m_session->GetAccountId(); }
	bool CanStake(bool showmsgs);
	bool StakeClaim();
	bool IsInClaim(bool showmsgs);
	void LearnMissingSpells();
	void UnstuckAccount();
	bool MassSummEligible;
	bool RingCheck();
	bool bLight;
	bool bDark;
	bool bNeutral;
	bool GuildCreateEligible;
	uint32 gm_command_delay;		//delay placeholder for command parsing
	bool bADMINTagOn;				//GMTag++ for admins
	bool bSUSPENDED;				//Active suspension or not
	bool bPWEligible;				//Eligible for a password change?
	bool ColoredText;				//Toggle flag for administration colored chat
	bool AdminDND;					//Toggle for Admins to block GM whispers
	bool HasRecWhisp;				//flag for first receieved whisper - to send a warning about passwords
	set<Player *> gmTickets;		//tracks tickets GMs have opened
	bool ViewingTicket;				//This GM currently has a ticket open
	bool bEVADE;
	uint32 m_Unstuck;				//unstuck delay timer
	uint32 m_nextCooldown;			//.clearcooldown delay timer
	uint32 m_GMTagOff;				//delay for GMs turning GM tag off(so they cant see enemy player and turn off)
	bool CastSpellOnSelf(uint32 spellid);
	bool UW_Invisible;
	void RemoveAurasUW();
	void DoGMFreeze();
	const char* GetNameClick(const char * color = "");
	uint32 AddLootBars();
	bool AllowedPortTo(uint32 mapid, bool include_msg);
	...
	//only use on login, else use bFROZEN
	bool IsGMFrozen();
	bool bFROZEN;
	bool bHover;
	bool GoingToCreature;
	PlayClaim * m_stake;

	ARCEMU_INLINE void HonestGM(bool group)
	{
		if(m_session->m_gmData->rank < RANK_COADMIN) // So far only use this function on donators(GM tag or not)
		{
			Neutralize(false, false);
			if(group)
				BroadcastMessage("Player and GM mixed group detected.  Deactivating all cheats and GM modifications.  If you have any residual stacked buffs from .cheat stack you are required to remove them or be subject to disciplinary action.");
		}
		else
		{
			if(group)
				BroadcastMessage("NOTICE: As a member of UNDERWORLD Administration you have just bypassed a cheat check.  Please do not abuse this feature.");
		}
	}
	//Use in conjunction with GM freeze spells
	ARCEMU_INLINE void Neutralize(bool root = true, bool spellpower = true)
	{
		CooldownCheat		    = false;	
		CastTimeCheat		    = false;	
		PowerCheat			    = false;	
		...
		SetFloatValue(OBJECT_FIELD_SCALE_X, (float)1 );
		if (root)
		...
		fly << GetNewGUID();
		fly << uint32(5);
		SendMessageToSet(&fly, true);
		RemoveAurasUW();
		DeMorph();

		if (spellpower)
		{
			//reset spell power for when entering a pvp area
			for( uint8 school = 1; school < 7; ++school )
			{
				ModUnsigned32Value( PLAYER_FIELD_MOD_DAMAGE_DONE_POS + school, 0 );
			}
		}

		SetPlayerSpeed(RUN, 7);
		SetPlayerSpeed(SWIM, 6);
		SetPlayerSpeed(RUNBACK, 7 / 2); // Backwards slower, it's more natural :P
		SetPlayerSpeed(FLY, 14); // Flying is faster :P
	}
