...
#define DEF_VALUE_NOT_SET 0xDEADBEEF
		...
		static const char* default_underworld_config_file = "configs/underworld.conf";
#else
...
bool Master::Run(int argc, char ** argv)
{
	...
	char * underworld_config_file = (char*)default_underworld_config_file;
	...
	if( do_check_conf )
	{
		...
		Log.Notice( "Config", "Checking config file:: %s\n", underworld_config_file);
		if(Config.UnderworldConfig.SetSource(underworld_config_file, true) )
			Log.Success( "Config", "Passed without errors.\n");
		else
			Log.Warning( "Config", "Encountered one or more errors.\n");
	...
	if(Config.UnderworldConfig.SetSource(underworld_config_file))
		Log.Success( "Config", ">> configs/underworld.conf" );
	else
	{
		Log.Error( "Config", ">> configs/underworld.conf" );
		return false;
	}
