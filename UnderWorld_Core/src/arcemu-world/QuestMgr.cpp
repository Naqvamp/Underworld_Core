...
{
	uint32 retgold = qst->reward_money;
	if((plr->GetUInt32Value(PLAYER_FIELD_COINAGE) + retgold) > 2000000000)
	{
		switch( plr->AddLootBars() )
		{
		case 1: //no space
			{
				sChatHandler.SystemMessageToPlr(plr, "Your gold total cannot be reset until you free some space in your invenory for Gold Bars. Your gold total can never exceed 200,000!");
			}break;
		case 2: //We added 4 bars to inventory successfully
			{
				uint32 current_gold = plr->GetUInt32Value( PLAYER_FIELD_COINAGE );
				uint32 gross_total  = current_gold + retgold;
				retgold = ( gross_total - 2000000000);
			}break;
		}
	}
	return retgold;
