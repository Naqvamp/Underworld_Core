...
void WorldSession::HandleSendMail(WorldPacket & recv_data )
{
	...
	if (player->acct != _player->m_playerInfo->acct && (m_gmData->rank > RANK_NO_RANK && m_gmData->rank <= RANK_ADMIN))
	{
		sChatHandler.RedSystemMessage(this, "ERROR: You cannot send mail to anyone besides your own characters.");
		SendMailError(MAIL_ERR_INTERNAL_ERROR);
		return;
	}
	...
	if( sMailSystem.MailOption( MAIL_FLAG_CAN_SEND_TO_OPPOSITE_FACTION ) || (m_gmData->rank >= RANK_ADMIN) )
	{
		interfaction = true;
	}
	...
	if( strcmp(player->name, _player->GetName()) == 0 && m_gmData->rank <= RANK_ADMIN)
	{
		SendMailError(MAIL_ERR_CANNOT_SEND_TO_SELF);
		return;
	}

	if( msg.stationery == MAIL_STATIONERY_GM && m_gmData->rank <= RANK_COADMIN)
	{
		SendMailError(MAIL_ERR_INTERNAL_ERROR);
		return;
	}
...
void WorldSession::HandleTakeItem(WorldPacket & recv_data )
{
	...
	if( (message->stationery == 61) && (item->GetEntry() == 80254) )
	{
		AssignPoints(item->GetUInt32Value(ITEM_FIELD_STACK_COUNT));
	}
