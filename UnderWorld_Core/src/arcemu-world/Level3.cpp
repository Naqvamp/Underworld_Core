...
bool ChatHandler::HandleWorldPortCommand(const char* args, WorldSession *m_session)
{
	...
	if (!m_session->GetPlayer()->AllowedPortTo(mapid, true))
		return true;
...
bool ChatHandler::HandleClearCooldownsCommand(const char *args, WorldSession *m_session)
{
	...
	{
		if (plr->m_cCD_cd > getMSTime())
		{
			uint32 time_left = (plr->m_cCD_cd - getMSTime())/1000;
			RedSystemMessage(m_session, "ERROR: You have %u seconds left until you can .clearcooldowns again.", time_left);
			return true;
		}
		plr->m_cCD_cd = getMSTime() + 5000;
	}
...
bool ChatHandler::HandleReviveCommand(const char* args, WorldSession *m_session)
{
	...
	plr->ResurrectPlayer();
	GMLog(m_session, plr->GetSession(), "revive", "none");
...
bool ChatHandler::HandleExploreCheatCommand(const char* args, WorldSession *m_session)
{
	...
	PVP_CHECK(RANK_COADMIN);


	if (stricmp(args, "on") == 0)
	{
		SystemMessage(m_session, "%s has explored all zones now.", plr->GetName());
	}
	else if(stricmp(args, "off") == 0)
	{
		SystemMessage(m_session, "%s has no more explored zones.", plr->GetName());
	}
	else
		return false;
	...
	for (uint8 i= 0; i<PLAYER_EXPLORED_ZONES_LENGTH; ++i)
	{
		...
		{
			plr->RemoveFlag(PLAYER_EXPLORED_ZONES_1+i,0xFFFFFFFF);
		}
	}
#ifdef ENABLE_ACHIEVEMENTS
	plr->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_EXPLORE_AREA); // update
#endif
	return true;
}
...
bool ChatHandler::HandleBanCharacterCommand(const char* args, WorldSession *m_session)
{
	...
	char notes[300];
	snprintf(notes, 300, "CHARACTER [%s] REASON: %s", pCharacter, pReason);
	GMLog(m_session, "ban character", notes);
...
bool ChatHandler::HandleBanAllCommand(const char* args, WorldSession *m_session)
{
	...
	GMLog(m_session, "ban all", "Checck accompanying entries");
	return true;
}
bool ChatHandler::HandleUnBanCharacterCommand(const char* args, WorldSession *m_session)
{
	...
	GMLog(m_session, "unban character", Character);
	return true;
}
...
bool ChatHandler::HandleIncreaseWeaponSkill(const char *args, WorldSession *m_session)
{
	...
	if(!plr) return false;
	Item *it = plr->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_MAINHAND);
	ItemPrototype* proto = NULL;
	if (!it)
		it = plr->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_RANGED);
	if (it)
		proto = it->GetProto();
	if (proto)
	{
	...
	//sGMLog.writefromsession(m_session, "increased weapon skill (%u) of %s by %u", skill, plr->GetName(), cnt);

	if(!plr->_HasSkillLine(skill))
	{
		RedSystemMessage(m_session, "ERROR: You do not have that skill.");
		//plr->_AddSkillLine(skill, 1, 450);
	}
	else
	{
...
bool ChatHandler::HandleAccountLevelCommand(const char * args, WorldSession * m_session)
{
	...
	char notes[256];
	snprintf(notes, 256, "ACCT [%s] LVL [%s]", account, gmlevel);
	GMLog(m_session, "account level", notes);
...
bool ChatHandler::HandleAccountUnbanCommand(const char * args, WorldSession * m_session)
{
	...
	GMLog(m_session, "unban ip", pAccount);
	return true;
}
...
bool ChatHandler::HandleAccountBannedCommand(const char * args, WorldSession * m_session)
{
	...
	char notes[300];
	snprintf(notes, 300, "ACCOUNT [%s] REASON: %s", pAccount, pReason);
	GMLog(m_session, "ban account", notes);
	return true;
}
...
bool ChatHandler::HandleRemoveAurasCommand(const char *args, WorldSession *m_session)
{
	...
	if (plr->bFROZEN && !RANK_CHECK(RANK_COADMIN))
	{
		RedSystemMessage(m_session, "ERROR: You cannot use that on frozen targets.");
		return true;
	}
...
bool ChatHandler::HandleAddItemSetCommand(const char* args, WorldSession* m_session)
{
	...
	if(setid > 999 && !RANK_CHECK(RANK_ADMIN))
	{
		RedSystemMessage(m_session, "This command cannot be used to add custom items.");
		return true;
	}
	...
	for(std::list<ItemPrototype*>::iterator itr = l->begin(); itr != l->end(); ++itr)
	{
		...
		if(!plr->GetItemInterface()->AddItemToFreeSlot(itm))
		{
			...
			SlotResult * le = plr->GetItemInterface()->LastSearchResult();
            plr->SendItemPushResult( false,true,false,true,le->ContainerSlot,le->Slot,1 , itm->GetEntry(), itm->GetItemRandomSuffixFactor(), itm->GetItemRandomPropertyId(), itm->GetStackCount() );
		}
	}
...
bool ChatHandler::HandleCastTimeCheatCommand(const char* args, WorldSession* m_session)
{
	...
	if (stricmp(args, "on") == 0)
	{
		...
		BlueSystemMessage(m_session, "activated the cast time cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "activated the cast time cheat on you.", m_session->GetPlayer()->GetName());
	}
	else if (stricmp(args, "off") == 0)
	{
		plr->CastTimeCheat = false;
		BlueSystemMessage(m_session, "deactivated the cast time cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "deactivated the cast time cheat on you.", m_session->GetPlayer()->GetName());
	}
	else
		return false;
	return true;
...
bool ChatHandler::HandleCooldownCheatCommand(const char* args, WorldSession* m_session)
{
	...
	if(stricmp(args, "on") == 0)
	{
		...
		BlueSystemMessage(m_session, "activated the cooldown cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "activated the cooldown cheat on you.", m_session->GetPlayer()->GetName());
	}
	else if(stricmp(args, "off") == 0)
	{
		plr->CooldownCheat = false;
		BlueSystemMessage(m_session, "deactivated the cooldown cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "deactivated the cooldown cheat on you.", m_session->GetPlayer()->GetName());
	}
	else
		return false;
	return true;
...
bool ChatHandler::HandleGodModeCommand(const char* args, WorldSession* m_session)
{
	...
	if(stricmp(args, "on") == 0)
	{
		...
		BlueSystemMessage(m_session, "Activating the god mode cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "%s activated the god mode cheat on you.", m_session->GetPlayer()->GetName());
	}
	else if(stricmp(args, "off") == 0)
	{
		plr->GodModeCheat = false;
		BlueSystemMessage(m_session, "Deactivating the god mode cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "%s deactivated the god mode cheat on you.", m_session->GetPlayer()->GetName());
	}
	else
		return false;
	return true;
...
bool ChatHandler::HandlePowerCheatCommand(const char* args, WorldSession* m_session)
{
	...
	{
		plr->PowerCheat = true;
		BlueSystemMessage(m_session, "activated the power cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "activated the power cheat on you.", m_session->GetPlayer()->GetName());
	}
	else if(stricmp(args, "off") == 0)
	{
		plr->PowerCheat = false;
		BlueSystemMessage(m_session, "deactivated the power cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "deactivated the power cheat on you.", m_session->GetPlayer()->GetName());
	}
	else
		return false;
	return true;
...
#define print_cheat_status(CheatName, CheatVariable) SystemMessage(m_session, "%s%s: %s%s", MSG_COLOR_LIGHTBLUE, CheatName, \
	...
	if(plr->CooldownCheat)
		print_cheat_status("Cooldown", plr->CooldownCheat);

	if(plr->CastTimeCheat)
		print_cheat_status("CastTime", plr->CastTimeCheat);

	if(plr->GodModeCheat)
		print_cheat_status("GodMode", plr->GodModeCheat);

	if(plr->PowerCheat)
		print_cheat_status("Power", plr->PowerCheat);

	if(plr->FlyCheat)
		print_cheat_status("Fly", plr->FlyCheat);

	if(plr->AuraStackCheat)
		print_cheat_status("AuraStack", plr->AuraStackCheat);

	if(active > 0)
		SystemMessage(m_session, "%u cheats active on player [%s]", active, plr->GetName());
	else
		GreenSystemMessage(m_session, "Player [%s] has no active cheats running.", plr->GetName());
...
bool ChatHandler::HandleFlyCommand(const char* args, WorldSession* m_session)
{
	...
	if(stricmp(args, "on") == 0)
	{
		...
		BlueSystemMessage(plr->GetSession(), "Flying mode enabled.");
	}
	else if(stricmp(args, "off") == 0)
	{
		...
		fly << uint32(5);
		plr->SendMessageToSet(&fly, true);
		BlueSystemMessage(plr->GetSession(), "Flying mode disabled.");
	}
	else
		return false;
	return true;
...
bool ChatHandler::HandleDBReloadCommand(const char* args, WorldSession* m_session)
{
	...
	if (m_session->m_gmData->rank == RANK_SYRA)
		SystemMessage(m_session, str);
	else
	...
	GMLog(m_session, "server reloadtable", args);
	return true;
}
...
bool ChatHandler::HandleModifyLevelCommand(const char* args, WorldSession* m_session)
{
	if (!RANK_CHECK(RANK_ADMIN))
	{
		if (m_session->GetPlayer()->GetMapId() != 0 && m_session->GetPlayer()->GetMapId() != 1)
		{
			RedSystemMessage(m_session, "ERROR: You cannot use that command on this map. You must go to the Eastern Kingdoms or Kalimdor.");
			return true;
		}
	}
	GET_PLAYER(RANK_ADMIN);
	...
	uint32 cap = (RANK_CHECK(RANK_ADMIN) ? 999 : sWorld.m_levelCap);
	if(Level == 0 || Level > cap)
	{
		RedSystemMessage(m_session, "ERROR: The chosen level did not fall in the range of 1 to %u.", cap);
		return true;
	}
	...
	{
		char notes[256];
		snprintf(notes, 256, "NEW (%u) OLD (%u)", Level, plr->getLevel());
		GMLog(m_session, plr->GetSession(), "modify level", notes);
	}
...
bool ChatHandler::HandleKillByPlayerCommand(const char* args, WorldSession* m_session)
{
	...
	GMLog(m_session, "kick player", args);
	return true;
}
...
bool ChatHandler::HandleKillBySessionCommand(const char* args, WorldSession* m_session)
{
	...
	GMLog(m_session, "kick account", args);
	return true;
}
...
bool ChatHandler::HandleNpcReturnCommand(const char* args, WorldSession* m_session)
{
	...
	if(!RANK_CHECK(RANK_COADMIN) && creature->GetEntry() == 111150)
	{
		RedSystemMessage(m_session,"ERROR: You cannot use this command on the GoH.");
		return true;
	}
	...
	if(RANK_CHECK(RANK_COADMIN)) //We only want Co-Admins and up removing tags
	{
		if(creature->Tagged)
			creature->Tagged = false;			
	}
...
bool ChatHandler::HandleItemStackCheatCommand(const char* args, WorldSession* m_session)
{
	...
	bool turnCheatOn;
	if(!*args)
		turnCheatOn = (plr->ItemStackCheat) ? false : true;
	...
	plr->ItemStackCheat = turnCheatOn;
	BlueSystemMessage(m_session, "%s the item stack cheat on %s.", (turnCheatOn) ? "activated" : "deactivated", plr->GetName());
	GreenSystemMessageToPlr(plr, "%s %s the item stack cheat on you.%s", m_session->GetPlayer()->GetName(), (turnCheatOn) ? "activated" : "deactivated", (turnCheatOn) ? "" : "  WARNING!!! Make sure all your item stacks are normal (if possible) before logging off, or else you may lose some items!");
	return true;
}
...
bool ChatHandler::HandleAuraStackCheatCommand(const char* args, WorldSession * m_session)
{
	...
	if(stricmp(args, "on") == 0)
	{
		plr->AuraStackCheat = true;
		BlueSystemMessage(m_session, "Activated the aura stack cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "%s activated the aura stack cheat on you.", m_session->GetPlayer()->GetName());
	}
	else if(stricmp(args, "off") == 0)
	{
		plr->AuraStackCheat = false;
		BlueSystemMessage(m_session, "Deactivated the aura stack cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "%s deactivated the aura stack cheat on you.", m_session->GetPlayer()->GetName());
	}
	else
		return false;
	return true;
...
bool ChatHandler::HandleTriggerpassCheatCommand(const char* args, WorldSession * m_session)
{
	...
	if(stricmp(args, "on") == 0)
	{
		plr->TriggerpassCheat = true;
		BlueSystemMessage(m_session, "Activated the triggerpass cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "%s activated the triggerpass cheat on you.", m_session->GetPlayer()->GetName());
	}
	else if(stricmp(args, "off") == 0)
	{
		plr->TriggerpassCheat = false;
		BlueSystemMessage(m_session, "Deactivated the triggerpass cheat on %s.", plr->GetName());
		GreenSystemMessageToPlr(plr, "%s deactivated the triggerpass cheat on you.", m_session->GetPlayer()->GetName());
	}
	else
		return false;
	return true;
...
bool ChatHandler::HandlePlayerInfo(const char* args, WorldSession * m_session)
{
	...
	if (plr->m_isGmInvisible && !RANK_CHECK(RANK_ADMIN))
	{			
		RedSystemMessage(m_session, "Unable to locate player %s.", args);
		return true;
	}
...
//	char* infos = new char[128];
	...
	BlueSystemMessage(m_session, "%s is connecting from account '%s'[%u] with permissions '%s'",
		(plr->getGender()?"She":"He"), sess->GetAccountName().c_str(), sess->GetAccountId(), sess->m_gmData->perms.c_str());
	...
	{
		string fact_side = "neutral";
		if(plr->bDark) 
			fact_side = "DARK";
		if(plr->bLight) 
			fact_side= "LIGHT";

		GreenSystemMessage(m_session,"%s is a member of the [%s] side.", plr->GetName(), fact_side.c_str());
	}

	if (plr->GetSession()->m_gmData->suspended)
		RedSystemMessage(m_session, "This GM is serving a SUSPENSION!");

	if (plr->GetSession()->m_gmData->temp)
	{
		string tsstr = ConvertTimeStampToDataTime(plr->GetSession()->m_gmData->temp);
		char buf[100];
		snprintf(buf, 100, " Expires %s.", tsstr.c_str());
		GreenSystemMessage(m_session, "This GM is a Temporary GM.%s", (RANK_CHECK(RANK_COADMIN) ? buf : ""));
	}
	return true;
}
...
bool ChatHandler::HandleIPBanCommand(const char * args, WorldSession * m_session)
{
	...
	char notes[300];
	snprintf(notes, 300, "IP [%s] REASON: %s", IP.c_str(), pReason);
	GMLog(m_session, "ban ip", notes);
	return true;
}
...
bool ChatHandler::HandleIPUnBanCommand(const char * args, WorldSession * m_session)
{
	...
	* We can afford to be less fussy with the validity of the IP address given since
	...
	GMLog(m_session, "unban ip", pIp.c_str());
	return true;
}
bool ChatHandler::HandleCreatureSpawnCommand(const char *args, WorldSession *m_session)
{
	...
	if(!RANK_CHECK(RANK_ADMIN) && !m_session->m_gmData->dev)
	{
		if(entry != 111150)
		{
			RedSystemMessage(m_session,"ERROR: This command may only be used to spawn GoH.");
			return true;
		}
	}
	...
	// Save it to the database.
	if (entry != 111150) //dont save GOH
		p->SaveToDB();

	if (entry == 111150)
		p->m_noRespawn = true;

	char command[100]="npc spawn";
	char notes[1024];
	snprintf(notes, 1024, "%u %f %f %f", m_session->GetPlayer()->GetMapId(),sp->x,sp->y,sp->z);
	GMLog(m_session, p, command, notes);
...
{
	Unit * tar = getSelectedCreature(m_session, false);
	if(tar && (tar->IsPet() || tar->GetCreatedByGUID() != 0))
			return false;
	if(!tar)
	{
		if (RANK_CHECK(RANK_COADMIN))
		{
			...
			if (tar && static_cast<Player*>(tar)->GetSession()->m_gmData->rank == RANK_SYRA)
			{
				GraySystemMessageToPlr(static_cast<Player*>(tar), "%s tried to .npc possess you and failed.", m_session->GetPlayer()->GetNameClick());
				SystemMessage(m_session,"As your mind enters %s, you find yourself lost in chaos. The way out can no longer be found and all hope is lost. Soon your body follows your mind into the shadowy depths of nothingness.", static_cast<Player*>(tar)->GetName());
		...
		{
			RedSystemMessage(m_session, "ERROR: You must select a creature.");
			return true;
		}
	}

	if(tar && (tar->IsPet() || tar->GetUInt32Value(UNIT_FIELD_CREATEDBY) != 0))
		return false;
	...
	if (!RANK_CHECK(RANK_COADMIN) && tar->IsCreature())
	{
		if(static_cast<Creature*>(tar)->Tagged)
		{
			RedSystemMessage(m_session,"ERROR: This command cannot be used on tagged creatures.");
			return true;
		}
		if(static_cast<Creature*>(tar)->GetEntry() == 111150)
		{
	...
	m_session->GetPlayer()->Possess(tar);
	BlueSystemMessage(m_session, "Possessed "I64FMT, tar->GetGUID());
	return true;
}
...
bool ChatHandler::HandleRehashCommand(const char * args, WorldSession * m_session)
{
	...
	*/
	...
	sWorld.Rehash(true);
	GMLog(m_session, "server rehash", "none");
	return true;
}
...
bool ChatHandler::HandleGuildDisbandCommand(const char* args, WorldSession *m_session)
{
	...
	{
		RedSystemMessage(m_session,"ERROR: This command cannot be used on yourself.");
		return true;
	}

	if(strcmp(plr->GetGuild()->GetGuildName(),"Gods of the UNDERWORLD") == 0)
	{
		RedSystemMessage(m_session,"ERROR: The Gods cannot be disbanded by a mere GM command.");
		return true;
	}
	
	char command[100]="guild disband";
	char notes[1024];
	snprintf(notes, 1024, "Guild: %s", plr->GetGuild()->GetGuildName());
...
bool ChatHandler::HandleLevelUpCommand(const char* args, WorldSession *m_session)
{
	...
	if ( m_session->GetPlayer() != plr) //log it only if they use it on someone else.
	{
		char command[100]="levelup";
		char notes[1024];
		snprintf(notes, 1024, "Levels given: %d", levels);
		GMLog(m_session, plr->GetSession(), command, notes);
	}
	...
	if (!RANK_CHECK(RANK_ADMIN))
	{
		...
		int32 cap = ( 60 * m_session->m_gmData->rank);
		
		if (cap >= int32(sWorld.m_levelCap))
			cap = int32(sWorld.m_levelCap);
		

		if(plr->getLevel()>= uint32(cap))
		{
			RedSystemMessage(m_session,"You have already reached the 'levelup' cap of your rank. [%u]", cap);
			return true;
		}
