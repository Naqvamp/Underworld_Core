...
public:
	...
	***********************************************/
	bool totemattempt;
	bool floodMuteCheck;
	void UpdateTempGM(bool login);
	GMStatus * m_gmData; //replaces Player::GMRank and the other bools
	uint32 m_LastAcctUnstuck;
	int8 AssignReward(uint32 itemid);
	UWFlags * m_uwflags;

	string HandleChatColor(string msg, GMRank rank);
	bool UWParseMsg(string & msg);
	void Mute(const char * reason, WorldSession * m_session, const char * nameB = "");
	void SendShowBank( uint64 guid );
	void SendMailbox();
	void SendAuctionListUW(uint32 entry);
	void StopAttack();

	uint32 m_points;
	void LoadPoints();
	void SavePoints();
	void AssignPoints(int32 count);
	void GMGearCheck();
	bool LogPurchase(char purchase[1024], uint32 price);
	//this must be called BERFORE deleting permissions!
	bool LogTempExpire();
	void SetSecurity(std::string securitystring, bool reloadforced);
