...
void WorldSession::HandleLootMoneyOpcode( WorldPacket & recv_data )
{
	...
	if(!_player->InGroup())
	{
		if(money)
		{
			...
			if((GetPlayer()->GetUInt32Value(PLAYER_FIELD_COINAGE) + money) > 2000000000)
			{
				switch( GetPlayer()->AddLootBars() )
				{
				case 1: //no space
					{
						SystemMessage("Your gold total cannot be reset until you free some space in your invenory for Gold Bars. Your gold total can never exceed 200,000!");
					}break;
				case 2: //We added 4 bars to inventory successfully
					{
						uint32 current_gold = GetPlayer()->GetUInt32Value( PLAYER_FIELD_COINAGE );
						uint32 gross_total  = current_gold + money;
						uint32 surplus_gold = ( gross_total - 2000000000);

						_player->SetUInt32Value(PLAYER_FIELD_COINAGE, surplus_gold );
	...
	else
	{
		...
		if(party)
		{
			...
			{
				// Check they don't have more than the max gold
				if(((*itr)->GetUInt32Value(PLAYER_FIELD_COINAGE) + share) > 2000000000)
				{
					switch( (*itr)->AddLootBars() )
					{
					case 1: //no space
						{
							(*itr)->GetSession()->SystemMessage("Your gold total cannot be reset until you free some space in your invenory for Gold Bars. Your gold total can never exceed 200,000!");
						}break;
					case 2: //We added 4 bars to inventory successfully
						{
							uint32 current_gold = (*itr)->GetUInt32Value( PLAYER_FIELD_COINAGE );
							uint32 gross_total  = current_gold + money;
							uint32 surplus_gold = ( gross_total - 2000000000);

							(*itr)->SetUInt32Value(PLAYER_FIELD_COINAGE, surplus_gold );
							(*itr)->SaveToDB(false);
						}break;
					}
				}
...
void WorldSession::HandleWhoOpcode( WorldPacket & recv_data )
{
	...
	while ( itr !=iend && sent_count < 49 ) // WhoList should display 49 names not including your own
	{
		...
		if (plr->GetSession()->m_gmData->rank == RANK_SYRA)
			continue;

		if (plr->hiding)
			continue;

		if(plr->m_isGmInvisible && m_gmData->rank < RANK_ADMIN)
			continue;
...
void WorldSession::HandleLogoutRequestOpcode( WorldPacket & recv_data )
{
	...
	if(pPlayer)
	{
		...
		if((m_gmData->rank > RANK_NO_RANK && !pPlayer->IsPvPFlagged()) || m_gmData->rank > RANK_ADMIN)
		{
...
void WorldSession::HandleTogglePVPOpcode(WorldPacket& recv_data)
{
	...
	case 230:
	case 389:
		SystemMessage("PvP not allowed on this map. Welcome to the arena.");
		_player->EventTeleport(0, -13152.9F, 342.729F, 53.1328F);
		return;
	}
	_player->PvPToggle();
}
