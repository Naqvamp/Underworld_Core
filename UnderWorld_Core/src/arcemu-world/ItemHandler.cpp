...
void WorldSession::HandleDestroyItemOpcode( WorldPacket & recv_data )
{
	...
	if(it)
	{
		...
		if (m_gmData->rank < RANK_COADMIN)
			switch (it->GetProto()->ItemId)
			{
			case 190004: //basic telebook	
				if (_player->GetItemInterface()->GetItemCount(190003, true))
					break; //if they have the advenced let them delete

			case 138000: //player rulebook
			case 138001: //gm rule book
			case 190003: //adv telebook
				SystemMessage("You cannot delete [%s].", it->GetProto()->Name1);
				return;
...
//	int8 bagslot = 0;
	...
	/*were buying an item and we could have set the item past its db set max stack size...
	uint32 itemMaxStack = (_player->ItemStackCheat) ? 0x7fffffff : it->MaxCount;
