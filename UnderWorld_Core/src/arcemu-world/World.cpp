...
World::World()
{
	...
	blockOutlands = false;
	customItemCutoff = 0;
	enableIntroVid = false;
	
	arenaEventInProgress = false;
	arenaEventTimeout = 0;
...
void World::Update(time_t diff)
{
	...
	//Have to add this here until i can figure out why the damn event crashes the server
	if (arenaEventInProgress  && arenaEventTimeout < (uint32)UNIXTIME) //Runtime of this check won't have any impact on the server
		ToggleArenaProgress(NULL, false); //This has the same runtime as a staff message announce. Minuscule and will run very rarely in anycase
...
void World::SendGamemasterMessage(WorldPacket *packet, WorldSession *self)
{
	...
	for(itr = m_sessions.begin(); itr != m_sessions.end(); itr++)
	{
	  ...
	  && itr->second != self)  // don't send to self!
	  {
		if(itr->second->m_gmData->rank > RANK_NO_RANK)
		itr->second->SendPacket(packet);
	  }
	}
	m_sessionlock.ReleaseReadLock();
...
void World::Rehash(bool load)
{
	if(load)
	{
		...
		Config.UnderworldConfig.SetSource("configs/underworld.conf", true);
		#else
	...
	//UNDERWORLD
	dmg_cap = Config.UnderworldConfig.GetIntDefault("Server", "DamageCap", 0);
	ap_modifier = Config.UnderworldConfig.GetFloatDefault("Player", "APModifier", 0.0F);
	sp_modifier = Config.UnderworldConfig.GetFloatDefault("Player", "SPModifier", 0.0F);
	enableIntroVid = Config.UnderworldConfig.GetBoolDefault("Player", "EnableIntroVid", false);
	dark_light_enabled = Config.UnderworldConfig.GetIntDefault("Server", "DARK_LIGHT", 0);
	dark_lang = Config.UnderworldConfig.GetIntDefault("Server", "DarkLang", 0);
	light_lang = Config.UnderworldConfig.GetIntDefault("Server", "LightLang", 0);
	dodge_ratio_level = Config.UnderworldConfig.GetFloatDefault("Server","DodgeRatioLevel", 2.2F);
	dodge_ratio_agi = Config.UnderworldConfig.GetFloatDefault("Server","DodgeRatioAgi", 250);
	spell_haste_cap = Config.UnderworldConfig.GetIntDefault("Server","SpellHasteCap", 250);
	blockOutlands = Config.UnderworldConfig.GetBoolDefault("Server","BlockOutlands", true);
	customItemCutoff = Config.UnderworldConfig.GetIntDefault("Server","CustomItemIds", 45000);
	logonDB = Config.UnderworldConfig.GetStringDefault("Server", "LogonDatabase", "z_logon");
	resilCap = Config.UnderworldConfig.GetIntDefault("Player", "ResilienceCap", 10000);
	realmID = Config.UnderworldConfig.GetIntDefault("Server", "Realm", 0);
	...
	m_limits.enable = Config.MainConfig.GetBoolDefault("Limits", "Enable", false);
	...
	m_limits.broadcast = Config.MainConfig.GetBoolDefault("Limits", "BroadcastGMs", false);
