...
public:
	...
	void AddItemBox(uint8 Icon, std::string Message, int32 Id, std::string BoxMessage, uint32 BoxMoney, bool Coded = false);
...
public:
	...
	//UNDERWORLD
protected:
	set<uint32> m_non_scaling_spells;
	
public:
	ARCEMU_INLINE bool IsSpellNonScaling(uint32 spellid)
	{
		return m_non_scaling_spells.find(spellid) != m_non_scaling_spells.end();
	}
	void LoadNonScalingSpells();
