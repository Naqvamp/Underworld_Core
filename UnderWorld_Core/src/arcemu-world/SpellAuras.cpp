...
void Aura::Remove()
{
	...
	if(m_target && m_target->IsPlayer() && GetSpellProto())
	{
		Player *plyr = static_cast<Player*>(m_target);
		if(!plyr)return;

		if(GetSpellId() == 17691 || GetSpellId() == 9454)
		{
			plyr->bFROZEN=false;
			plyr->SetMovement(MOVE_UNROOT,5);
			if(plyr->HasAura(50224))
				plyr->RemoveAura(50224);
			plyr->BroadcastMessage("GM incapacitation removed.  You must relog to remove any residual effects.");
		}
	}
...
void Aura::SpellAuraDummy(bool apply)
{
	...
	case 12295: // Tactical Mastery
	case 12676:
	case 12677:
	case 12678: // Stance Mastery
		{
			if( p_target == NULL )
				return;
			if(apply)
