...
// revision:		$Rev: 3138 $
// date:		$Date: 2010-01-29 10:57:59 -0500 (Fri, 29 Jan 2010) $
...
void Unit::Strike( Unit* pVictim, uint32 weapon_damage_type, SpellEntry* ability, int32 add_damage, int32 pct_dmg_mod, uint32 exclusive_damage, bool disable_proc, bool skip_hit_check, bool force_crit, bool skip_in_back )
{
//==========================================================================================
...
//==========================================================================================
	...
	if(!skip_in_back && !isInFront(pVictim))
	{
		if(IsPlayer())
		{
...
void Unit::AddAura(Aura * aur)
{
	...
	if((m_mapId!=530 && m_mapId!=571 && m_mapId!=1 && m_mapId!=0) || (IsPlayer() && static_cast<Player*>(this)->IsPvPFlagged()))
	...
	if( !aur->IsPassive() )
	{
		...
		if( aur->GetSpellProto()->always_apply == false )
		{
			...
			uint32 totalAuras = 0;//track the number of applied auras
			...
			if (IsPlayer())
				if (totalAuras > 100 && !deleteAur) //breaking 100 buffs wil be a LOT harder now but we will check for it anyways
					//if we are already deleting the aura lets not do this stuff
				{
					Player * plr = static_cast<Player*>(this);
					plr->RemoveAurasUW();
					sChatHandler.RedSystemMessageToPlr(plr, "The power of your buffs has destroyed you.");
...
int32 Unit::GetSpellDmgBonus(Unit *pVictim, SpellEntry *spellInfo,int32 base_dmg, bool isdot)
{
	...
	if( caster->IsPlayer() )
	{
		...
		if (objmgr.IsSpellNonScaling(spellInfo->Id))
			return 0;
	}
...
//------------------------------by school----------------------------------------------
	...
	if (!(spellInfo->c_is_flags & SPELL_FLAG_IS_HEALING) && sWorld.sp_modifier != 0.0F)
		res = int32(float(res * sWorld.sp_modifier));

	if (sWorld.realmID & REALM_EVO)
		res *= 10;
...
Creature* Unit::create_guardian(uint32 guardian_entry,uint32 duration,float angle, uint32 lvl, GameObject * obj, LocationVector * Vec)
{
	...
	if (duration > 0) //let me do 'perm' guardians >.>
