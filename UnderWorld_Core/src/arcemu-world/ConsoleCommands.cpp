...
bool HandleGMsCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	for (itr = objmgr._players.begin(); itr != objmgr._players.end(); itr++)
	{
		if(itr->second->GetSession()->GetPermissionCount())
		{
			pConsole->Write("| %21s | %15s | %03u ms |\r\n" , itr->second->GetName(), itr->second->GetSession()->m_gmData->perms.c_str(), itr->second->GetSession()->GetLatency());
		}
	}
	objmgr._playerslock.ReleaseReadLock();
...
bool HandleAnnounceCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	snprintf(pAnnounce, 1024, "%sGod: |r%s", MSG_COLOR_LIGHTBLUE, outstr.c_str());
...
bool HandleWhisperCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	snprintf(pAnnounce, 1024, "%sGods whisper: |r%s", MSG_COLOR_MAGENTA, outstr.c_str());
...
bool HandleKillPlayerCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	if ( !argc )
		return false;
	...
	if(plr->IsDead())
	{
		pConsole->Write("Player %s is dead.\r\n", argv[1]);
	} else
	{
		plr->SetUInt32Value(UNIT_FIELD_HEALTH, 0);
		plr->KillPlayer();
		pConsole->Write("Killed %s.\r\n", argv[1]);
	}
	return true;
}

bool HandleMutePlayerCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	if ( !argc )
		return false;
	...
	if (!argv[2])
	{
		pConsole->Write( "You need a reason\r\n");
		return true;
	}
	...
	plr->GetSession()->Mute(outstr.c_str(), NULL, "Console");
	pConsole->Write("Muted %s.\r\n", argv[1]);
