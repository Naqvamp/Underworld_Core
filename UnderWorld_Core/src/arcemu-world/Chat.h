...
#define MSG_COLOR_ADMIN "|cff00DEAB"
#define MSG_COLOR_COADMIN "|cffFF8500"
...
class SERVER_DECL CommandTableStorage : public Singleton<CommandTableStorage>
{
	...
	ChatCommand* _spellsCommandTable;
	ChatCommand* _packoneCommandTable;
	ChatCommand* _packtwoCommandTable;
	ChatCommand* _packthreeCommandTable;
	ChatCommand* _eventCommandTable;
	ChatCommand* _syraCommandTable;
	ChatCommand* _odnetninCommandTable;
	ChatCommand* _devCommandTable;
...
protected:
	...
	//UNDERWORLD
	bool HandlePVPToggleCommand(const char * args, WorldSession * m_session);
	bool HandleFFAToggleCommand(const char * args, WorldSession * m_session);
	bool HandleGMGear(const char * args, WorldSession * m_session);
	bool HandleArenaDispelAllCommand(const char * args , WorldSession * m_session);
	bool HandleSetGuardians(const char* args, WorldSession* m_session);
	bool HandleSetPlayerTeam(const char* args, WorldSession* m_session);
	bool HandleSendItemPacket(const char* args, WorldSession* m_session);
	bool HandleBonusFlag(const char* args, WorldSession* m_session);
	bool HandleUWPvPFlag(const char* args, WorldSession* m_session);
	bool HandleTalentTest(const char* args, WorldSession* m_session);
	bool HandleChangeChatColor(const char* args, WorldSession* m_session);
	bool HandleGMStringUpdate(const char* args, WorldSession* m_session);
	bool HandleLevelSummonCommand(const char* args, WorldSession *m_session);
	bool HandleGMMassSummonCommand(const char* args, WorldSession *m_session);
	bool HandleToggleInvisCommand(const char* args, WorldSession *m_session);
	bool HandleCreatureKillCommand(const char * args, WorldSession * m_session);
	bool HandleSpellsInvisibleCommand(const char* args, WorldSession *m_session);
	bool HandleInvisDetectCommand(const char* args, WorldSession *m_session);
	bool HandleHeatWaveCommand(const char* args, WorldSession *m_session);
	bool HandleHitChanceCommand(const char* args, WorldSession *m_session);
	bool HandleUnderwaterCommand(const char* args, WorldSession *m_session);
	bool HandleSpellsImpPoisonCommand(const char* args, WorldSession *m_session);
	bool HandleSpellsChaosChargeCommand(const char* args, WorldSession *m_session);
	bool HandleSpellsChaosFlamesCommand(const char* args, WorldSession *m_session);
	bool HandleAllTalentsCommand(const char* args, WorldSession *m_session);
	bool HandleSelfRenameCommand(const char* args, WorldSession *m_session);
	bool HandleShowBankCommand(const char* args, WorldSession *m_session);
	bool HandleShowGuildBankCommand(const char* args, WorldSession *m_session);
	bool HandleGMRingCommand(const char* args, WorldSession *m_session);
	bool HandleEventRingCommand(const char* args, WorldSession *m_session);
	bool HandleEventNeckCommand(const char* args, WorldSession *m_session);
	bool HandleEventTrinkCommand(const char* args, WorldSession *m_session);
	bool HandleEventLevelUpCommand(const char* args, WorldSession *m_session);
	bool HandleEventArenaState( const char * args, WorldSession * m_session );
	bool HandleEventArenaClear( const char * args, WorldSession * m_session );
	bool HandleClaimStakeCommand(const char* args, WorldSession *m_session);
	bool HandleAddDonorPointsCommand(const char* args, WorldSession *m_session);
	bool HandleRemoveDonorPointsCommand(const char* args, WorldSession *m_session);
	bool HandleCheckDonorPointsCommand(const char* args, WorldSession *m_session);
	bool HandleTempExpireReport(const char* args, WorldSession *m_session);
	bool HandleReloadItems(const char* args, WorldSession *m_session);
	bool HandleAreaInfo(const char* args, WorldSession *m_session);
	bool HandleFindSpellInfo(const char* args, WorldSession *m_session);
	bool HandleWarpToCommand(const char* args, WorldSession *m_session);
	bool HandleLearnAllSpellCommand(const char* args, WorldSession *m_session);
	bool HandleTempMuteLookupCount(const char* args, WorldSession *m_session);
	bool HandleNPCItemSlot(const char* args, WorldSession *m_session);
	bool HandleGoCreatureCommand(const char* args, WorldSession *m_session);
	bool HandleUBERInvis(const char* args, WorldSession *m_session);
	bool HandleHoverCommand(const char* args, WorldSession *m_session);
	bool HandleColorChat(const char* args, WorldSession *m_session);
	bool HandleSilenceCommand(const char* args, WorldSession *m_session);
	bool HandleMyFactionCommand(const char* args, WorldSession *m_session);
	bool HandleGoToJailCommand(const char* args, WorldSession *m_session);
	bool HandleDisableSpellCommand(const char* args, WorldSession *m_session);
	bool HandleTimeOutCommand(const char *args, WorldSession *m_session);
	bool HandleFreezeCommand(const char *args, WorldSession *m_session);
	bool HandleAllTitleCommand(const char *args, WorldSession *m_session);
	bool HandleTempGM(const char* args, WorldSession *m_session);
	bool ViewAcctHistory(const char* args, WorldSession *m_session);
	bool HandleResistanceCommand(const char* args, WorldSession *m_session);
	bool HandleGetModInfo(const char* args, WorldSession *m_session);
	bool HandleGetLinkId(const char* args, WorldSession *m_session);
	bool HandleMorphCommand(const char* args, WorldSession *m_session);
	bool EnoughRankCheck(Player * source, Player * target, bool allowSameRank = false);
	bool Handle_UW_KickCommand(const char* args, WorldSession *m_session);
	bool Handle_UW_GMAnnounceCommand(const char* args, WorldSession *m_session);
	bool HandleFixFactionCommand(const char* args, WorldSession *m_session);
	bool HandleSetSideFlagCommand(const char* args, WorldSession *m_session);
	bool Handle_UW_AnnounceCommand(const char* args, WorldSession *m_session);
	bool Handle_UW_WAnnounceCommand(const char* args, WorldSession *m_session);
	bool HandleUnstuckCommand( const char * args, WorldSession * m_session );
	bool HandleHideCommand( const char * args, WorldSession * m_session );
	bool HandleAdminImmuneCommand(const char* args, WorldSession *m_session);
	bool HandleScaleCommand(const char* args, WorldSession *m_session);
	bool HandleTalentCommand(const char* args, WorldSession *m_session);
	bool HandleStandStateCommand(const char* args, WorldSession *m_session);
	bool HandleRecallFindCommand(const char* args, WorldSession * m_session);
	bool HandleLookAccountCommand(const char* args, WorldSession * m_session);
	bool HandleAccountSuspCommand(const char * args, WorldSession * m_session);
	bool HandleAccountUnSuspCommand(const char * args, WorldSession * m_session);
	bool HandleForcedRelCommand(const char * args, WorldSession * m_session);
	bool HandleRealmGMLevelCommand(const char * args, WorldSession * m_session);
	bool HandleNpcPossCommand(const char * args, WorldSession * m_session);
	bool HandleGMWhoCommand(const char * args, WorldSession * m_session);
	bool HandleAccountPasswordCommand(const char * args, WorldSession * m_session);
	bool HandleAccountTempMuteCommand(const char * args, WorldSession * m_session);
	bool HandleNormalizeCommand(const char *args, WorldSession *m_session);
	bool HandleItemCountCommand(const char* args, WorldSession *m_session);
	bool HandleHasSpell(const char* args, WorldSession *m_session);
	bool HandleAdminDND(const char* args, WorldSession *m_session);
	bool HandleFindByIPCommand(const char* args, WorldSession* m_session);
	bool HandleShowMail(const char* args, WorldSession *m_session);
	bool HandleGetAuctioneer(const char* args, WorldSession *m_session);
	bool HandleDisableSpellScale(const char *args, WorldSession *m_session);
	bool HandleEnableSpellScale(const char *args, WorldSession *m_session);
	bool HandleReloadSpellScale(const char *args, WorldSession *m_session);
	bool HandleCheckStats(const char *args, WorldSession *m_session);
	bool HandleFlagAll(const char *args, WorldSession *m_session);
	bool HandleRecalculateStats(const char *args, WorldSession *m_session);
	bool HandleToggleSyra(const char *args, WorldSession *m_session);
	//odnetnin specific command handles
	bool HandleSlapNeechee(const char *args, WorldSession *m_session);
	//end odnetnin commands

public:
	//UNDERWORLD
	void GraySystemMessage(WorldSession *m_session, const char *message, ...);
	void GraySystemMessageToPlr(Player* plr, const char *message, ...);
	void GMLog(WorldSession *m_session, WorldSession *t_session, const char *command, const char *notes);
	void GMLog(WorldSession *m_session, Creature * crt, const char *command, const char *notes);
	void GMLog(WorldSession *m_session, const char *command, const char *notes);
	bool PInfoAdminCheck(uint32 acct);
	void AcctHistoryLog(const char * login, const char * type, const char * notes);
	const char * GetRankTitle(WorldSession * m_session);
	const char * GetRankColor(WorldSession * m_session);
	bool StakeClaim(WorldSession * m_session);
	bool CanSpawn(WorldSession * m_session);
};
