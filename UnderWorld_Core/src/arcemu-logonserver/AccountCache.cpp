...
void AccountMgr::ReloadAccounts(bool silent)
{
	...
	// Load *all* accounts.
	QueryResult * result = sLogonSQL->Query("SELECT acct, login, password, encrypted_password, gm, flags, banned, forceLanguage, muted, suspended FROM accounts");
	Field * field;
	string AccountName;
...
void AccountMgr::AddAccount(Field* field)
{
	...
	acct->Suspended = field[9].GetUInt32();
