	...
	case 8:		// set password
		{
			...
			// update it in the sql (duh)
			sLogonSQL->Execute("UPDATE accounts SET password = \"%s\" WHERE login = \"%s\"", sLogonSQL->EscapeString(password).c_str(), sLogonSQL->EscapeString(account).c_str());
		}break;


	case 9:		// set suspension
		{
			string account;
			uint32 duration;
			string reason;
			...
			pAccount->Suspended = duration;
			

			// update it in the sql (duh)
			sLogonSQL->Execute("UPDATE accounts SET suspended = %u, suspReason = \"%s\" WHERE login = \"%s\"", duration, sLogonSQL->EscapeString(reason).c_str(),  sLogonSQL->EscapeString(account).c_str());
		}break;
	}
