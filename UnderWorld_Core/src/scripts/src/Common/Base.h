...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Class EncounterMgr
class EncounterMgr : std::map< uint32, Creature* >
{
public:
    EncounterMgr(Creature* pCreature) : m_master(pCreature) {};
    ~EncounterMgr() {};

    // spawns and auto adds creature to map
    Creature* Summon(uint32 uiEntry, float fX, float fY, float fZ, float fO, uint32 uiPhase = 1 );

    // adds creature in world to map
    void AddCreature( Creature* pCreature );

    // removes creature from map
    void RemoveCreature(Creature* pCreature, bool bDespawn = true ){ RemoveCreature( pCreature->GetLowGUID(), bDespawn ); };
    void RemoveCreature(uint32 uiLowGUID, bool bDespawn = true );
    
    // removes all creatures with uiEntry
    void RemoveAllCreaturesEntry( uint32 uiEntry, bool bDespawn = true );

    // cleares whole map
    void RemoveAllCreatures( bool bDespawn = true );
...
{
public:
	SpellDesc(SpellEntry* pInfo, SpellFunc pFnc, TargetType pTargetType, float pChance, float pCastTime, int32 pCooldown, float pMinRange, float pMaxRange, bool pStrictRange, char* pText, TextType pTextType, uint32 pSoundId, char* pAnnouncement );
	virtual ~SpellDesc();

	EmoteDesc*	AddEmote(const char* pText, TextType pType=Text_Yell, uint32 pSoundId=0);
	void		TriggerCooldown(uint32 pCurrentTime=0);
	void		AddAnnouncement( char* pText );
	...
	char*		mAnnouncement;		//Announce spell cast
...
public:
	...
	//Spells
	SpellDesc*				AddSpell(uint32 pSpellId, TargetType pTargetType, float pChance, float pCastTime, int32 pCooldown, float pMinRange=0, float pMaxRange=0, bool pStrictRange=false, char* pText=NULL, TextType pTextType=Text_Yell, uint32 pSoundId=0, char* pAnnouncement=NULL );
	SpellDesc*				AddSpellFunc(SpellFunc pFnc, TargetType pTargetType, float pChance, float pCastTime, int32 pCooldown, float pMinRange=0, float pMaxRange=0, bool pStrictRange=false, char* pText=NULL, TextType pTextType=Text_Yell, uint32 pSoundId=0, char* pAnnouncement=NULL );
