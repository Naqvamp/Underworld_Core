...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Class EncounterMgr
Creature* EncounterMgr::Summon(uint32 uiEntry, float fX, float fY, float fZ, float fO, uint32 uiPhase)
{
    Creature* pNewCreature = m_master->GetMapMgr()->GetInterface()->SpawnCreature( uiEntry, fX, fY, fZ, fO, true, false, 0, 0, uiPhase );
    if( !pNewCreature )
        return NULL;

    insert( std::pair< uint32, Creature* >( pNewCreature->GetLowGUID(), pNewCreature ) );
    return pNewCreature;
};

void EncounterMgr::AddCreature(Creature* pCreature)
{
    if( !pCreature )
        return;

    insert( std::pair< uint32, Creature* >( pCreature->GetLowGUID(), pCreature ) );
};

void EncounterMgr::RemoveCreature(uint32 uiLowGUID, bool bDespawn)
{
    iterator itr = find( uiLowGUID );
    erase(itr);

    if( bDespawn && itr->second )
        itr->second->Despawn( 1000, 0 );
};

void EncounterMgr::RemoveAllCreaturesEntry(uint32 uiEntry, bool bDespawn)
{
    for( iterator itr = begin(); itr != end(); ++itr )
    {
        ...
        if( itr->second->GetEntry() == uiEntry )
        {
            erase(itr);
            if( bDespawn )
                itr->second->Despawn( 1000, 0 );
...
void EncounterMgr::RemoveAllCreatures(bool bDespawn)
{
    for( iterator itr = begin(); itr != end(); ++itr )
    {
        if( !itr->second )
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Class SpellDesc
SpellDesc::SpellDesc(SpellEntry* pInfo, SpellFunc pFnc, TargetType pTargetType, float pChance, float pCastTime, int32 pCooldown, float pMinRange, float pMaxRange, bool pStrictRange, char* pText, TextType pTextType, uint32 pSoundId, char* pAnnouncement)
{
	mInfo = pInfo;
	mSpellFunc = pFnc;
...
void SpellDesc::AddAnnouncement(char *pText)
{
	mAnnouncement = ( pText && strlen(pText) > 0 ) ? pText : const_cast< char* >( "" );
}
...
SpellDesc* MoonScriptCreatureAI::AddSpell(uint32 pSpellId, TargetType pTargetType, float pChance, float pCastTime, int32 pCooldown, float pMinRange, float pMaxRange, bool pStrictRange, char* pText, TextType pTextType, uint32 pSoundId, char* pAnnouncement)
{
...
SpellDesc* MoonScriptCreatureAI::AddSpellFunc(SpellFunc pFnc, TargetType pTargetType, float pChance, float pCastTime, int32 pCooldown, float pMinRange, float pMaxRange, bool pStrictRange, char* pText, TextType pTextType, uint32 pSoundId, char* pAnnouncement)
{
	if( !pFnc ) return NULL;
...
void MoonScriptCreatureAI::Announce( char* pText )
{
	if( pText && strlen(pText) > 0 )
