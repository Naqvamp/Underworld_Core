...
bool SunspringBurn(uint32 i, Spell * pSpell)
{
 if( !pSpell->p_caster )
  return true;

 Unit * target = pSpell->GetUnitTarget();
 if( !target || target->GetEntry() != 18240)
  return true;

 ((Creature*)target)->Despawn(500, 360000);

 Player * pPlayer = pSpell->p_caster;
 QuestLogEntry * en = pPlayer->GetQuestLogForEntry( 9874 );
 if( en && en->GetMobCount( 0 ) < en->GetQuest()->required_mobcount[0] )
	{
  en->SetMobCount( 0, en->GetMobCount( 0 ) + 1 );
  en->SendUpdateAddKill( 0 );
...
void SetupQuestItems(ScriptMgr * mgr)
{
	...
	mgr->register_dummy_spell(32146, &SunspringBurn);			//http://www.wowhead.com/?item=24560
