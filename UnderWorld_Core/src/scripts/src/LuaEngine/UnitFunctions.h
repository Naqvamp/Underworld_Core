...
namespace luaUnit
{
	...
	int GetInventoryItemById(lua_State * L, Unit * ptr)
	{
		...
		if (slot == -1) //check bags
		{
			for (uint8 contslot = INVENTORY_SLOT_BAG_START; contslot != INVENTORY_SLOT_BAG_END; contslot++)
			{
				...
				{
					if (bag->GetItem(bslot)->GetEntry() == entry)
					{
	...
	{
		CHECK_TYPEID(TYPEID_PLAYER);
		const char * aName = TO_PLAYER(ptr)->GetSession()->GetAccountName().c_str();
		if(aName != NULL)
...
*/
	...
	int SetChannelOwner(lua_State * L, Unit * ptr)
	{
		CHECK_TYPEID(TYPEID_PLAYER);
		const char* channel_name = luaL_checkstring(L, 1);
		Player* plr = ((Player*)ptr);
		Channel* pChannel = channelmgr.GetChannel(channel_name, plr); // player must be in channel to become leader
		if(!channel_name || !plr || !pChannel)
			return 0;
		pChannel->SetOwner(pChannel->GetOwner(), plr);
		return 1;
	}

	int GetChannelOwner(lua_State * L, Unit * ptr)
	{
		CHECK_TYPEID(TYPEID_PLAYER);
		const char* channel_name = luaL_checkstring(L, 1);
		Player* plr = ((Player*)ptr);
		...
		if(!channel_name || !plr || !pChannel)
			return 0;
		ArcLuna<Player>::push(L, pChannel->GetOwner());
		return 1;
	}

	int SendPacketToChannel(lua_State * L, Unit * ptr)
	{
		CHECK_TYPEID(TYPEID_PLAYER);
		const char* channel_name = luaL_checkstring(L, 1);
		int buffer_index = luaL_checkint(L, 2);
		Channel* pChannel = channelmgr.GetChannel(channel_name, ((Player*)ptr));
		if(!channel_name || !ptr || !pChannel)
			return 0;
		pChannel->SendToAll(&Bufferdata[buffer_index]);
	...
	int GetChannelOwner(lua_State * L, Unit * ptr)
	{
		...
		if(!ptr || !pChannel)
			return 0;
		ArcLuna<Player>::push(L, pChannel->GetOwner(((Player*)ptr)));
		return 1;
	}
	...
	int GetChannelMembers(lua_State * L, Unit * ptr)
	{
		CHECK_TYPEID(TYPEID_PLAYER);
		const char* channel_name = luaL_checkstring(L, 1);
		if(!ptr || !channel_name)
		...
		if(pChannel)
		{
			for(MemberMap::iterator itr = pChannel->m_members.begin(); itr != pChannel->m_members.end(); ++itr)
			{
				if((*itr)->m_loggedInPlayer)
				{
					count++;
					ret = (*itr)->m_loggedInPlayer;
					lua_pushinteger(L, count);
	...
	int GetAura(lua_State * L, Unit * ptr)
	{
		...
		if (slot > MAX_TOTAL_AURAS_START && slot < MAX_TOTAL_AURAS_END)
			lua_pushnumber(L, ptr->m_auras[slot]->GetSpellId());
		return 1;
	}
	...
	{
		if (!ptr) return 0;
		lua_pushboolean(L,(ptr->isRooted())?1:0);
		return 1;
	}
	...
	int HasAuraWithMechanic(lua_State * L, Unit * ptr)
	{
		...
		if(ptr != NULL && mechanic )
			lua_pushboolean(L, (ptr->HasAuraWithMechanics(mechanic)) ? 1 : 0);
		else
			lua_pushnil(L);
		return 1;
	}
	...
	int SetActionBar(lua_State * L, Unit * ptr) //experimental
	{
		CHECK_TYPEID(TYPEID_PLAYER);
		Player * plr = TO_PLAYER(ptr);
	...
	int GetClosestEnemy(lua_State * L, Unit * ptr)
	{
		...
		for (set<Object*>::iterator itr = ptr->GetInRangeSetBegin(); itr != ptr->GetInRangeSetEnd(); ++itr)
		{
			if (!(*itr)->IsUnit() || isFriendly((*itr), ptr))
				continue;
			Unit * pUnit = TO_UNIT(*itr);
			d2 = pUnit->GetDistanceSq(ptr);
			if (d2<dist)
			{
				dist=d2;
				ret=pUnit;
