...
Position ProspectorAnvilwardWaypoints[] = {
	...
	switch ( IntId )
	{
		case 1:
		{
			...
			if ( qLogEntry != NULL )
			{
				...
				for ( uint32 i = 0; i < sizeof( ProspectorAnvilwardWaypoints ) / sizeof( Position ); i++ )
				{
					wp = new WayPoint;
					wp->id = i + 2;
...
class ProspectorAnvilwardAI : public CreatureAIScript
{
	public:
		...
		{
			if ( iWaypointId == sizeof( ProspectorAnvilwardWaypoints ) / sizeof( Position ) && bForwards )
			{
