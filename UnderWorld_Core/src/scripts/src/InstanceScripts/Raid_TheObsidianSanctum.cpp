...
public:
    ...
    void OnCreatureDeath( Creature* pVictim, Unit* pKiller )
    {
        ...
        {
        case CN_DRAKE_TENEBRON:
            m_creatureGuid[DRAKE_TENEBRON] = 0;
            break;
        case CN_DRAKE_VESPERON:
            m_creatureGuid[DRAKE_VESPERON] = 0;
            break;
        case CN_DRAKE_SHADRON:
            m_creatureGuid[DRAKE_SHADRON] = 0;
            break;
        case CN_SARTHARION:
    ...
    void DoDrakeAura( uint8 pData )
    {
        ...
        switch( pData )
        {
            case CN_DRAKE_TENEBRON: pSpellEntry = TENEBRON_AURA; break;
            case CN_DRAKE_SHADRON: pSpellEntry = SHADRON_AURA; break;
            case CN_DRAKE_VESPERON: pSpellEntry = VESPERON_AURA; break;
            default:
            break;
        };
    ...
    Creature* GetCreature( uint8 pData )
    {
        ...
        return m_creatureGuid[pData] == 0 ? NULL : GetCreatureByGuid( m_creatureGuid[pData] ); 
    };
};

#define GET_OS(ptr) static_cast<  ObsidianSanctumScript* >( ptr->GetMapMgr()->GetScript() )
