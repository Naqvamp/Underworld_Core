...
static Coords MoltenGolemCoords[]= 
{
...
class Volkhan : public MoonScriptCreatureAI
{
	...
	Volkhan(Creature* pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		m_cVolkhanWP.mX = 1328.666870f;       
		m_cVolkhanWP.mY = -97.022758f;
		m_cVolkhanWP.mZ = 56.675297f;
		m_cVolkhanWP.mO = 2.235341f;
...
class MoltenGolem : public MoonScriptCreatureAI
{
	...
	uint32 OnDamageTaken(Unit* mAttacker, uint32 Amount, uint32 SpellId)
	{
		if ( Amount >= _unit->GetUInt32Value( UNIT_FIELD_HEALTH ) )
		{
...
bool Temper(uint32 i, Spell *pSpell)
{
	...
	for ( int i = 0; i < 2; i++)
	{
		...
		pCreature = pMap->SpawnCreature( CN_MOLTEN_GOLEM, MoltenGolemCoords[i].mX, MoltenGolemCoords[i].mY, MoltenGolemCoords[i].mZ, MoltenGolemCoords[i].mO, true, true, 0, 0, 1 );
		if ( pCreature )
