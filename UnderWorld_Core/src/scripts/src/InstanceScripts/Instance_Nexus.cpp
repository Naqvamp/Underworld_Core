...
public:
    ...
    void AIUpdate()
    {
		if((GetHealthPercent() <= 75 && mSummon == 0) 
		|| (GetHealthPercent() <= 50 && mSummon == 2) 
		|| (GetHealthPercent() <= 25 && mSummon == 4))
			mSummon += 1;
		
		if(mSummon == 1 || mSummon == 3 || mSummon == 5) 
			ChargeRift();
    ...
    void SummonRift( bool bToCharge )
    {
        ...
        SpawnCreature( CN_CHAOTIC_RIFT, _unit->GetPositionX() + 13.5f, _unit->GetPositionY(), _unit->GetPositionZ(), _unit->GetOrientation(), false );
    };
