...
-- Spell Targeting of Aura of Death needs to be scripted for Mark of Death to work properly
function DoomWalker_Earthquake(unit)
	unit:CastSpell(32686)
	unit:SendChatMessage(14, 0, "Magnitude set. Release.")
	unit:PlaySoundToSet (11345)
end

function DoomWalker_overrun(unit)
	unit:CastSpell(32637)
	unit:SendChatMessage(14, 0, "Trajectory Locked.")
...
function DoomWalker_OnEnterCombat(unit)
	unit:SendChatMessage(14, 0, "Do not proceed. You will be eliminated.")
...
function DoomWalker_KilledTarget(unit,event,pVictim)
	if pVictim:IsPlayer() then
		unit:SendChatMessage(14, 0, "Target Exterminated.")
		unit:PlaySoundToSet (11351)
		unit:RemoveEvents()
		unit:FullCastSpellOnTarget(37128,pVictim)
	end
end

function DoomWalker_OnDied(unit)
	unit:SendChatMessage(14, 0, "System failure in five....four....")
...
RegisterUnitEvent(17711, 3, "DoomWalker_KilledTarget")
RegisterUnitEvent(17711, 4, "DoomWalker_OnDied") 
--RegisterUnitEvent(17711, 21, "DoomWalker_AIUpdate")